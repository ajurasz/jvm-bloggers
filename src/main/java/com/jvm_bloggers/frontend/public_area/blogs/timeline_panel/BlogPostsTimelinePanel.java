...
public class BlogPostsTimelinePanel extends Panel {
    ...
    private YearMonth currentYearMonth = YearMonth.from(LocalDate.now());
        ...
        final DataView<BlogPost> dataView =
            new DataView<BlogPost>(DATA_VIEW_ID, requestHandler) {
                ...
                protected void populateItem(Item<BlogPost> item) {
                    ...
                    item.add(new BlogPostDetailsPanel("blogPostDetails",
                        getOrientationCssClass(iterationCounter, yearMonthSeparatorPanel.isVisible()),
                        post));
                    iterationCounter++;
                }
            };
    ...
    private String getOrientationCssClass(int counter, boolean separatorVisible) {
        if (counter % 2 == 0) return LEFT_CSS_CLASS;
        if (counter % dataView.getItemsPerPage() == 0) return RIGHT_OFFSET_7_CSS_CLASS;
